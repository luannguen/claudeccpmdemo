/**
 * SaaS Module - Invoice Card Component
 * 
 * Displays invoice information with status and actions.
 * 
 * @module features/saas/ui/billing
 */
...
import { getDaysUntilDue, getInvoiceUrgency } from '../../domain/billingRules';
...
export default function InvoiceCard({ invoice, onView, onPay, onDownload }) {
  ...
  return (
    <Card className={`${urgency === 'critical' ? 'border-red-300 border-2' : ''}`}>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
              ...
              {invoice.status === 'paid' && <Icon.CheckCircle />}
              {invoice.status === 'overdue' && <Icon.AlertCircle />}
              {invoice.status !== 'paid' && invoice.status !== 'overdue' && <Icon.DollarSign />}
            </div>
            <div>
