...
import { Bookmark, AlertCircle, Sparkles, ArrowLeft } from "lucide-react";
...
export default function MySavedPosts() {
  ...
  const { data: savedPosts = [], isLoading: postsLoading } = useQuery({
    queryKey: ['user-saved-posts', currentUser?.email],
    queryFn: async () => {
      if (!currentUser?.email) return [];
      const allPosts = await base44.entities.UserPost.list('-created_date', 200);
      return allPosts.filter(p => {
        const data = p.data || p;
        return data.saved_by?.includes(currentUser.email) && data.status === 'active';
      });
    },
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-gray-600">{savedPosts.length} bài viết</p>
            </div>
          </div>
        </motion.div>
